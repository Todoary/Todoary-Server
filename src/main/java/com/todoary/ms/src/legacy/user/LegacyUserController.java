...
public class LegacyUserController {
    ...
    @Autowired
    public LegacyUserController(JwtTokenProvider jwtTokenProvider, PasswordEncoder passwordEncoder, LegacyUserService legacyUserService, LegacyUserProvider legacyUserProvider, LegacyUserDao legacyUserDao, AwsS3Service awsS3Service) {
    ...
    public BaseResponse<PatchUserRes> patchProfile(HttpServletRequest request, @RequestBody PatchUserReq patchUserReq) {
        try {
            ...
            PatchUserRes patchUserRes = legacyUserService.modifyProfile(user_id, patchUserReq);
    ...
    public BaseResponse<PostProfileImgRes> uploadProfileImg(@RequestParam("profile-img") MultipartFile multipartFile, HttpServletRequest request) {
        ...
        try {
            ...
            String profile_img_fileKey = legacyUserProvider.retrieveById(user_id).getProfile_img_url();
            ...
            return new BaseResponse<>(legacyUserService.setProfileImg(user_id, profile_img_url));
        } catch (BaseException e) {
    ...
    public BaseResponse<String> deleteProfileImg(HttpServletRequest request) throws BaseException {
        ...
        int result = 0;
        try {
            String filekey = legacyUserProvider.retrieveById(user_id).getProfile_img_url();
            ...
            filekey = legacyUserProvider.retrieveById(user_id).getProfile_img_url().substring(54);
            awsS3Service.fileDelete(filekey);
            legacyUserService.modifyProfileImgToDefault(user_id);
            return new BaseResponse<>("삭제에 성공하였습니다.");
        } catch (BaseException e) {
            ErrorLogWriter.writeExceptionWithAuthorizedRequest(e, request);
            return new BaseResponse<>(e.getStatus());
    ...
    public BaseResponse<GetUserRes> getProfile(HttpServletRequest request) {
        try {
            Long user_id = Long.parseLong(request.getAttribute("user_id").toString());
            User user = legacyUserProvider.retrieveById(user_id);
    ...
    public BaseResponse<BaseResponseStatus> patchUserStatus(HttpServletRequest request) {
        try {
            Long user_id = Long.parseLong(request.getAttribute("user_id").toString());
            legacyUserService.removeUser(user_id);
    ...
    public BaseResponse<BaseResponseStatus> logout(HttpServletRequest request) {
        try {
            ...
            Long user_id = Long.parseLong(request.getAttribute("user_id").toString());
            /* remove refreshToken */
            if (legacyUserProvider.checkRefreshToken(user_id) == 1) {
                legacyUserService.removeRefreshToken(user_id);
            }

            /* remove FCM_token */
            if (legacyUserProvider.checkFCMToken(user_id) == 1) {
                legacyUserService.removeFCMToken(user_id);
            }
    ...
    PatchAlarmReq patchAlarmReq) {
        try {
            Long user_id = Long.parseLong(request.getAttribute("user_id").toString());
            legacyUserService.modifyAlarm(user_id, "alarm_todo", patchAlarmReq.isChecked());
            return new BaseResponse<>(BaseResponseStatus.SUCCESS);
        } catch (BaseException e) {
    ...
    public BaseResponse<BaseResponseStatus> patchDiaryAlarmStatus(HttpServletRequest request, @RequestBody PatchAlarmReq patchAlarmReq) {
        try {
            Long user_id = Long.parseLong(request.getAttribute("user_id").toString());
            legacyUserService.modifyAlarm(user_id, "alarm_diary", patchAlarmReq.isChecked());
            return new BaseResponse<>(BaseResponseStatus.SUCCESS);
        } catch (BaseException e) {
            writeExceptionWithAuthorizedRequest(e, request, patchAlarmReq.toString());
            return new BaseResponse<>(e.getStatus());
    ...
    public BaseResponse<BaseResponseStatus> patchRemindAlarmStatus(HttpServletRequest request, @RequestBody PatchAlarmReq patchAlarmReq) {
        try {
            Long user_id = Long.parseLong(request.getAttribute("user_id").toString());
            legacyUserService.modifyAlarm(user_id, "alarm_remind", patchAlarmReq.isChecked());
            return new BaseResponse<>(BaseResponseStatus.SUCCESS);
        } catch (BaseException e) {
            writeExceptionWithAuthorizedRequest(e, request, patchAlarmReq.toString());
            return new BaseResponse<>(e.getStatus());
    ...
    public BaseResponse<GetAlarmEnabledRes> getAlarmEnabled(HttpServletRequest request) {
        try {
            ...
            legacyUserProvider.assertUserValidById(userId);
            return new BaseResponse<>(legacyUserProvider.retrieveAlarmEnabled(userId));
        } catch (BaseException e) {
            ErrorLogWriter.writeExceptionWithAuthorizedRequest(e, request);
            return new BaseResponse<>(e.getStatus());
        }
    ...
    public BaseResponse<BaseResponseStatus> patchTermsStatus(HttpServletRequest request, @RequestBody PatchTermsReq patchTermsReq) {
        try {
            Long user_id = Long.parseLong(request.getAttribute("user_id").toString());
            legacyUserService.serviceTerms(user_id, "terms", patchTermsReq.isChecked());
            return new BaseResponse<>(BaseResponseStatus.SUCCESS);
        } catch (BaseException e) {
    ...
    public void UserRemove() throws BaseException {
        ...
        try {
            legacyUserService.removeUserExpired(targetDate);
    ...
    public BaseResponse FCMTokenRefresh(HttpServletRequest request, @RequestBody PatchFcmTokenReq patchFcmTokenReq) {
        ...
        try {
            Long user_id = Long.parseLong(request.getAttribute("user_id").toString());
            legacyUserService.modifyFcmToken(user_id, fcm_token);
