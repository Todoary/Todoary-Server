...
public class JpaDiaryServiceTest {
    ...
    void Diary_생성() {
        ...
        //then
        diaryService.createOrModify(member.getId(),request,date);
    ...
    void Diary_수정() {
        ...
        diaryService.createOrModify(member.getId(),request,firstDate);
        // when
        String title = "title2";
        String content="content2";
        LocalDate modifiedDate = LocalDate.now();
        diaryService.createOrModify(member.getId(), new DiaryRequest(title, content), modifiedDate);
    ...
    void Diary_삭제() {
        ...
        assertThat(member.getDiary()).hasSize(0);
    }
    ...
    void Diary_조회() {
        ...
        LocalDate createdDate = LocalDate.of(2023, 1, 1);
        diaryService.createOrModify(member.getId(), request, createdDate);
        // when
        DiaryResponse diary = diaryService.findDiary(createdDate, member.getId());
        // then
    ...
    void 특정_달의_Diary가_있는_날짜들_조회() {
        ...
        Arrays.stream(dates)
                .forEach(date -> diaryService.createOrModify(
                        member.getId(),
                        new DiaryRequest("todo", "content"), date
                ));
        // when
        List<Integer> january = diaryService.findDiaryInMonth(member.getId(), YearMonth.of(2023, 1));
        List<Integer> february = diaryService.findDiaryInMonth(member.getId(), YearMonth.of(2023, 2));
        List<Integer> march = diaryService.findDiaryInMonth(member.getId(), YearMonth.of(2023, 3));
        // then
