...
class JpaCategoryControllerTest {
    ...
    void 카테고리_제목이_최대_길이_이하일때_생성O() throws Exception {
        ...
        // when
        CategorySaveRequest requestDto = new CategorySaveRequest("카테고리", 10);
        MvcResult result = mvc.perform(post("/v2/category/").requestAttr("user_id", 10L).with(csrf())
    ...
    void 카테고리_제목이_최대_길이_초과일때_생성X() throws Exception {
        ...
        CategorySaveRequest requestDto = new CategorySaveRequest(title, 10);
        // when
        MvcResult result = mvc.perform(post("/v2/category/").requestAttr("user_id", 10L).with(csrf())
                                               .contentType(MediaType.APPLICATION_JSON)
                                               .content(new ObjectMapper().writeValueAsString(requestDto)))
    ...
    void 카테고리_색상이_null일때_생성X() throws Exception {
        // given
        CategorySaveRequest requestDto = new CategorySaveRequest("title", null);
        // when
        MvcResult result = mvc.perform(post("/v2/category/").requestAttr("user_id", 10L).with(csrf())
                                               .contentType(MediaType.APPLICATION_JSON)
                                               .content(new ObjectMapper().writeValueAsString(requestDto)))
